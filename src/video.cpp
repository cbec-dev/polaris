...
namespace video {
  ...
  namespace {
    ...
    int nvenc_split_encode_mode_value(nvenc::nvenc_split_encode_mode mode) {
      switch (mode) {
        case nvenc::nvenc_split_encode_mode::disabled:
          return 15;
        case nvenc::nvenc_split_encode_mode::auto_mode:
          return 0;
        case nvenc::nvenc_split_encode_mode::forced:
          return 1;
        case nvenc::nvenc_split_encode_mode::two_way:
          return 2;
        case nvenc::nvenc_split_encode_mode::three_way:
          return 3;
      }
    ...
    bool should_apply_nvenc_split_encode_mode(std::string_view encoder_name, std::string_view codec_name) {
#ifdef __linux__
      return encoder_name == "nvenc"sv &&
             (codec_name == "hevc_nvenc"sv || codec_name == "av1_nvenc"sv);
#else
      return false;
#endif
    }

    std::optional<int> nvenc_split_encode_mode_option_value(
    ...
    bool avcodec_context_has_private_option(AVCodecContext *ctx, const char *name) {
      return ctx && av_opt_find(ctx, name, nullptr, 0, AV_OPT_SEARCH_CHILDREN) != nullptr;
    }

    void apply_nvenc_split_encode_mode(
      AVCodecContext *ctx,
    ...
    std::shared_ptr<platf::img_t> clone_dmabuf_compat_image(const frame_t &frame) {
      auto *src = dynamic_cast<egl::img_descriptor_t *>(frame.compat_img());
      if (!src) {
        BOOST_LOG(error) << "Expected EGL DMA-BUF compatibility image for Linux frame cloning"sv;
        return {};
      }

      auto clone = std::make_shared<egl::img_descriptor_t>();
      clone->data = src->data;
      clone->width = src->width;
      clone->height = src->height;
      clone->pixel_pitch = src->pixel_pitch;
      clone->row_pitch = src->row_pitch;
      clone->native_device_type = src->native_device_type;
      clone->native_resource = src->native_resource;
      clone->native_fd = src->native_fd;
      clone->frame_timestamp = src->frame_timestamp;
      clone->frame_metadata = src->frame_metadata;
      clone->x = src->x;
      clone->y = src->y;
      clone->src_w = src->src_w;
      clone->src_h = src->src_h;
      clone->serial = src->serial;
      clone->buffer = src->buffer;
      clone->sd = src->sd;
      clone->sequence = src->sequence;
      clone->dmabuf_buffer_key = src->dmabuf_buffer_key;

      for (auto &fd : clone->sd.fds) {
        if (fd < 0) {
          continue;
        }
        ...
        fd = dup_compat_fd(fd);
        if (fd < 0) {
          BOOST_LOG(error) << "Failed to duplicate DMA-BUF compatibility image fd for frame conversion"sv;
          return {};
        }
      }
    ...
    std::shared_ptr<platf::img_t> clone_compat_image_for_conversion(const frame_t &frame) {
      if (frame.source_metadata.transport != platf::frame_transport_e::dmabuf || !frame.compat_img()) {
        return {};
      }

      return clone_dmabuf_compat_image(frame);
    }
#else
    std::shared_ptr<platf::img_t> clone_compat_image_for_conversion(const frame_t &frame) {
      (void) frame;
    ...
    platf::frame_format_e frame_format_from_pix_fmt(platf::pix_fmt_e pix_fmt) {
      switch (pix_fmt) {
        case platf::pix_fmt_e::yuv420p:
          return platf::frame_format_e::yuv420p;
        case platf::pix_fmt_e::yuv420p10:
          return platf::frame_format_e::yuv420p10;
        ...
        case platf::pix_fmt_e::ayuv:
          return platf::frame_format_e::ayuv;
    ...
    platf::frame_residency_e frame_residency_from_mem_type(platf::mem_type_e mem_type) {
      switch (mem_type) {
        case platf::mem_type_e::system:
          return platf::frame_residency_e::cpu;
        ...
        default:
          return platf::frame_residency_e::unknown;
      }
    }

    std::string_view from_mem_type(platf::mem_type_e mem_type) {
      switch (mem_type) {
        case platf::mem_type_e::system:
          return "system"sv;
        case platf::mem_type_e::vaapi:
    ...
    std::optional<platf::pix_fmt_e> select_encoder_output_pix_fmt(const encoder_t &encoder, const config_t &config, const sunshine_colorspace_t &colorspace) {
      if (!encoder.platform_formats) {
        return std::nullopt;
      }

      if (config.chromaSamplingType == 1) {
        if (encoder.platform_formats->pix_fmt_yuv444_8bit == platf::pix_fmt_e::unknown &&
            encoder.platform_formats->pix_fmt_yuv444_10bit == platf::pix_fmt_e::unknown) {
          return std::nullopt;
        }

        return (colorspace.bit_depth == 10) ?
    ...
    std::optional<conversion_request_t> make_conversion_request(const encoder_t &encoder, const config_t &config, const sunshine_colorspace_t &colorspace, platf::mem_type_e target_device) {
      ...
      auto frame_format = frame_format_from_pix_fmt(*pix_fmt);
      if (frame_format == platf::frame_format_e::unknown) {
        return std::nullopt;
      }

      return conversion_request_t {
  ...
  class encode_device_frame_converter_t: public frame_converter_t {
  public:
    encode_device_frame_converter_t(std::string_view converter_name, std::unique_ptr<Device> device, conversion_request_t request):
    ...
    bool supports(const frame_t &src, const conversion_request_t &request) const override {
      return device_ &&
             src.valid() &&
             request.target_format == request_.target_format &&
             request.target_residency == request_.target_residency &&
             request.target_device == request_.target_device;
    }

    int convert(frame_t &frame, const conversion_request_t &request) override {
      if (!supports(frame, request)) {
        BOOST_LOG(error)
          << "Frame converter "sv << logging::bracket(std::string(converter_name))
          << " does not support frame conversion request: target_device="sv << from_mem_type(request.target_device)
          << " target_residency="sv << platf::from_frame_residency(request.target_residency)
          << " target_format="sv << platf::from_frame_format(request.target_format);
        return -1;
      }

      auto owned_compat_img = clone_compat_image_for_conversion(frame);
      auto *compat_img = frame.compat_img();
      if (owned_compat_img) {
        compat_img = owned_compat_img.get();
      } else if (frame.source_metadata.transport == platf::frame_transport_e::dmabuf && frame.compat_img()) {
        BOOST_LOG(error)
          << "Frame converter "sv << logging::bracket(std::string(converter_name))
          << " could not duplicate the DMA-BUF compatibility image for isolated conversion";
        return -1;
      }
      ...
      if (!compat_img) {
        BOOST_LOG(error)
          << "Frame converter "sv << logging::bracket(std::string(converter_name))
          << " requires a compatibility image for this conversion path";
        return -1;
      }
      ...
      auto native_device_type = device_->native_output_device_type();
      if (native_device_type != platf::mem_type_e::unknown) {
        frame.native.device_type = native_device_type;
      }
      frame.native.resource = device_->native_output_resource();
      frame.native.fd = device_->native_output_fd();
      if (request.target_residency == platf::frame_residency_e::gpu &&
          (frame.native.resource != nullptr || frame.native.fd >= 0)) {
  ...
  public:
    ...
    int convert(frame_t &frame) override {
      ...
      return converter->convert(frame, conversion_request);
    }

    platf::avcodec_encode_device_t *device() const {
      return converter ? converter->device() : nullptr;
    }

    void request_idr_frame() override {
      if (auto *encode_device = device(); encode_device && encode_device->frame) {
        auto &frame = encode_device->frame;
  ...
  public:
    ...
    platf::nvenc_encode_device_t *device() const {
      return converter ? converter->device() : nullptr;
    }
    ...
    void invalidate_ref_frames(int64_t first_frame, int64_t last_frame) override {
      ...
      if (!encode_device->nvenc->invalidate_ref_frames(first_frame, last_frame)) {
        force_idr = true;
      }
    }
    ...
    nvenc::nvenc_encoded_frame encode_frame(uint64_t frame_index) {
      ...
      auto result = encode_device->nvenc->encode_frame(frame_index, force_idr);
      force_idr = false;
      return result;
    }
  ...
  std::unique_ptr<avcodec_encode_session_t> make_avcodec_encode_session(
    ...
    for (int retries = 0; retries < 2; retries++) {
      ...
      // Allow platform options first, then apply user-level NVENC overrides.
      encode_device->init_codec_options(ctx.get(), &options);
      apply_nvenc_split_encode_mode(ctx.get(), encoder, video_format, &options);
    ...
    auto converter = std::make_unique<encode_device_frame_converter_t<platf::avcodec_encode_device_t>>(
      "avcodec_encode_device"sv,
      std::move(encode_device_final),
      *conversion_request
    );

    BOOST_LOG(info)
      << "Using frame converter "sv << logging::bracket(std::string(converter->name()))
  ...
  std::unique_ptr<nvenc_encode_session_t> make_nvenc_encode_session(const encoder_t &encoder, const config_t &client_config, std::unique_ptr<platf::nvenc_encode_device_t> encode_device) {
    ...
    auto conversion_request = make_conversion_request(
      ...
      encoder.platform_formats ? encoder.platform_formats->dev_type : platf::mem_type_e::unknown
    );
    if (!conversion_request) {
      BOOST_LOG(error) << "Could not derive nvenc conversion request from encoder configuration"sv;
      return nullptr;
    }

    auto converter = std::make_unique<encode_device_frame_converter_t<platf::nvenc_encode_device_t>>(
      "nvenc_encode_device"sv,
  ...
  encode_e encode_run_sync(
    ...
    auto initial_frame = make_frame(img);
  ...
  /**
   * @brief Load the cached encoder name from a previous successful probe.
...
namespace video {
  ...
  platf::mem_type_e active_encoder_mem_type() {
    ...
    return chosen_encoder->platform_formats->dev_type;
  }
  ...
  bool should_apply_nvenc_split_encode_mode_for_tests(
    ...
    return should_apply_nvenc_split_encode_mode(encoder_name, codec_name);
  }
