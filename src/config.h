...
namespace config {
  ...
  struct video_t {
    ...
    int hevc_mode;
    int av1_mode;
    int hdr_mode;  // 0 = off, 1 = auto, 2 = always enabled
    int nvenc_tune;  // NVENC tuning: 1 = high quality, 2 = low latency, 3 = ultra low latency
    ...
    int color_range;  ///< Color range override: 0 = client decides, 1 = force limited/MPEG, 2 = force full/JPEG

    struct linux_display_t {
      std::string streaming_output;   ///< Output name to enable for streaming (e.g. "HDMI-A-1")
      std::string primary_output;     ///< Main display to restore as primary after streaming (e.g. "DP-3")
      bool auto_manage_displays;      ///< Enable/disable automatic display management via kscreen-doctor
      bool use_cage_compositor;       ///< Launch games inside cage nested compositor for clean KMS capture
  ...
  struct recording_t {
    bool enabled = false;                ///< Master enable for recording features
    bool replay_buffer = false;          ///< If true, use replay buffer mode instead of continuous
    std::string output_dir;              ///< Where to save recordings (empty = appdata/recordings)
