...
namespace nvhttp {
   ...
   * @note The negative 4th number indicates to Moonlight that this is Polaris.
   */
  ...
  class PolarisHTTPS: public SimpleWeb::HTTPS {
  public:
    PolarisHTTPS(boost::asio::io_context &io_context, boost::asio::ssl::context &ctx):
  ...
  enum class PAIR_PHASE {
    NONE,  ///< Polaris is not in a pairing phase
    GETSERVERCERT,  ///< Polaris is in the get server certificate phase
    CLIENTCHALLENGE,  ///< Polaris is in the client challenge phase
    SERVERCHALLENGERESP,  ///< Polaris is in the server challenge response phase
