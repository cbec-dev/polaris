...
namespace config {
  ...
  namespace nv {
    ...
    nvenc::nvenc_split_encode_mode split_encode_mode_from_view(const ::std::string_view &mode) {
      if (mode == "disabled") {
        return nvenc::nvenc_split_encode_mode::disabled;
      }
      if (mode == "auto") {
        return nvenc::nvenc_split_encode_mode::auto_mode;
      }
      if (mode == "forced") {
        return nvenc::nvenc_split_encode_mode::forced;
      }
      if (mode == "2") {
        return nvenc::nvenc_split_encode_mode::two_way;
      }
      if (mode == "3") {
        return nvenc::nvenc_split_encode_mode::three_way;
      }
      BOOST_LOG(warning) << "config: unknown nvenc_split_encode_mode value: " << mode;
      return nvenc::nvenc_split_encode_mode::disabled;
    }
...
#undef _CONVERT_2_ARG_
  ...
  video_t video {
    ...
    24,  // qp (lower = better quality, 24 is visually superior with negligible perf cost on modern GPUs)

    0,  // hevc_mode
    0,  // av1_mode
    2,  // hdr_mode (2 = always)
    3,  // nvenc_tune (3 = ultra low latency)

    4,  // min_threads (better utilization on modern multi-core CPUs)
    {
      "superfast"s,  // preset
      "zerolatency"s,  // tune
      11,  // superfast
    ...
    0,  // color_range (0 = client decides, 1 = force limited, 2 = force full)

    {
      {},     // linux_display.streaming_output (empty = disabled)
      {},     // linux_display.primary_output (empty = disabled)
      ...
      false,  // linux_display.headless_vaapi_dmabuf_capture
    },  // linux_display
  ...
  stream_t stream {
    ...
    10,  // fecPercentage (10% is sufficient for wired LAN, saves bandwidth)
  ...
  sunshine_t sunshine {
    ...
    "ipv4",  // Address family
    platf::appdata().string() + "/polaris.log",  // log file
  ...
  recording_t recording {
    false,  // enabled
    false,  // replay_buffer
    {},     // output_dir (empty = appdata/recordings)
    5,      // replay_buffer_minutes
  };
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    #else
      BOOST_LOG(info) << "config: ["sv << name << "] -- ["sv << log_val << ']';
    #endif
    ...
    int_between_f(vars, "hdr_mode", video.hdr_mode, {0, 2});
    int_between_f(vars, "nvenc_tune", video.nvenc_tune, {1, 3});
    ...
    generic_f(vars, "nvenc_split_encode_mode", video.nv.split_encode_mode, nv::split_encode_mode_from_view);
    ...
    bool_f(vars, "linux_headless_vaapi_dmabuf_capture", video.linux_display.headless_vaapi_dmabuf_capture);
    ...
    string_f(vars, "api_key", config::sunshine.api_key);
    string_f(vars, "steamgriddb_api_key", config::sunshine.steamgriddb_api_key);
    ...
    // Recording / replay buffer options
    bool_f(vars, "recording_enabled", recording.enabled);
